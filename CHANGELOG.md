...
- **RIA accounts only** — rebalancing is an advisory proposal the client approves & executes, so the log is RIA-specific (**Send rebalance proposal**); PMS (discretionary, manager-executed) and International accounts don't show it. Seeded a realistic mixed history per funded RIA client.
...
- New **Monthly touch-base** panel — shows the **whole client book** sorted most-overdue-first, each with product, AUM, last contact, a **Status** chip (**Overdue · Nd / Due soon / Contacted · Nd ago**), a **contextual "why now" talking point** (e.g. "Portfolio below cost — reassure", "Quarterly review overdue", "Discuss tax-harvesting before FY-end", "SIP step-up opportunity"), and **📅 Schedule** (creates a 30-min touch-base call + Meet invite) / **✓ Log call** (records the contact — the client flips to a green *Contacted* status rather than vanishing). The **Touch-base due** tile counts only those ≥30 days since last contact. (Always populated — no empty dead-end when everyone's been contacted.)
