...
- **Rebalance history (panel).** Summary tiles: last successful rebalance date · total rebalances · failed/not-done count · next review. Event log columns: **Date · Status · What changed · By · Reason / action**. **Status** = ✓ Successful (Executed) / — Not done / ✕ Failed. Failed rows surface the failure reason (insufficient cash, exchange rejection, demat authorisation pending) + **Retry**; Not-done rows surface why (within tolerance band; RIA proposal not approved in the validity window). Action: **Send rebalance proposal** (RIA — client approves & executes) / **Record rebalance** (PMS — discretionary by Investment). [PM INPUT — true rebalance feed from the execution/OMS engine; this is illustrative in the demo.]
