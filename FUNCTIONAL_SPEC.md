...
Two worklists:
- **"Monthly touch-base — your client book"** (link *View all clients →* S-10). Shows the **whole funded book**, sorted most-overdue-first (so it's never an empty state). Columns: **Client (+ a contextual "why now" talking point) · Product · AUM · Last contact · Status · actions**. **Status** = Overdue · Nd (≥30d, red) / Due soon · Nd (21–29d, amber) / Contacted · Nd ago (<21d, green). The talking point is data-aware (e.g. portfolio below cost, review overdue) falling back to rotating examples (house-view, tax-harvesting, SIP step-up, liquidity check). Actions **📅 Schedule** / **✓ Log call**; logging flips the client to a *Contacted* status (it stays listed). The **Touch-base due** tile counts only Overdue (≥30d). Actions: **📅 Schedule** (creates a 30-min touch-base call + Meet invite, logged to the client's calls) · **✓ Log call** (records contact today, clearing them from the list). Row → Account Detail. Empty: "🎉 Everyone's been contacted this month."
...
**Tab 4 — Portfolio.** A reconciliation **suspense alert** when unreconciled positions exist (each: holding — amount · reason; action **Send repair notification** to the client). Metric cards: Investment amount · Current value · **Gain** (signed) · Absolute return % · IRR %. Holdings table: **Holding · Invested · Current value · Gain** (signed). Empty: "No holdings yet — client not funded." All figures: source Custodian (A1). Includes the **📋 Review prep** action (S-16 brief).
- **Rebalance history (panel — RIA accounts only).** Shown only for RIA-track accounts (rebalancing is an advisory **proposal** the client approves & executes); PMS is discretionary/manager-executed and International accounts don't show it. Summary tiles: last successful rebalance date · total rebalances · failed/not-done count · next review. Event log columns: **Date · Status · What changed · By · Reason / action**. **Status** = ✓ Successful (Executed) / — Not done / ✕ Failed. Failed rows surface the failure reason (insufficient cash, exchange rejection, demat authorisation pending) + **Retry**; Not-done rows surface why (within tolerance band; proposal not approved in the validity window). Action: **Send rebalance proposal**. [PM INPUT — true rebalance feed from the execution/OMS engine; illustrative in the demo.]
