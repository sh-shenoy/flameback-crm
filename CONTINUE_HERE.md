...
10. **Knowledge Hub** — a built-in docs/playbook view (`#view-hub`, in every role's nav) so anyone can look up how the CRM works. Sticky left table-of-contents (`hubGo(id)` smooth-scrolls to `#hub-*` sections) + 14 content sections: Overview, Teams & people (roster table), Who does what (duties per role), How a lead flows (L1–L4), Five-Strike Rule, App sign-ups & assignment, Onboarding & the form, Active clients, Tasks & handoffs, Compliance checks, Reviews & reminders, Leadership view, a "How do I…?" cheat-sheet, and a Glossary. **Content is plain static HTML inside the section** — easy to edit by hand. Styling via `.hub-*` classes. First draft for the user to review and revise.
